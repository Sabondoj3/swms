...
function WizardInner() {
  ...
  const [lat, setLat] = useState(26.1122288);
  const [lng, setLng] = useState(91.6520905);
  ...
  useEffect(() => {
    fetch("/api/points")
      ...
      .then((d) => {
        ...
        if (code) {
          const found = (d.points ?? []).find(
            (p: Point) => p.code === code
          );

          if (found) {
            setCollectionPointId(found.id);
  ...
  function useLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setError(
          "Unable to access your location. Please allow location permission or select the location on the map."
        );
      }
    );
  }
    ...
    return (
      <main className="mx-auto max-w-md px-4 py-10 text-center">
        <Card>
          ...
          <p className="mt-1 font-mono font-bold text-green-700">
            {done}
          </p>

          <p className="mt-2 text-sm text-slate-500">
            Admin will verify and assign a collector. Track progress
            in My Reports.
          </p>
        ...
        <Card className="mt-4 flex flex-col gap-3">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g. Behind Library, near gate"
            />
          </div>
        ...
        <Card className="mt-4 flex flex-col gap-3">
          ...
          {preview ? (
            ...
            <p className="text-sm text-slate-500">
              Photo helps collectors. You can submit without one,
            ...
            <div className="grid grid-cols-3 gap-2">
              {WASTE_CATEGORIES.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setWasteCategory(c.value)
                  }
                  className={`rounded-xl border p-3 text-xs font-semibold ${
                    wasteCategory === c.value
        ...
        <Card className="mt-4 flex flex-col gap-3">
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border px-4 py-3 text-sm"
              placeholder="Any helpful detail…"
            />
          </div>
