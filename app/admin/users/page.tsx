    ...
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mt-4 grid gap-2">
        {users.map((u) => <Card key={u.id} className="flex flex-wrap items-center justify-between gap-2">
          <div><p className="text-sm font-bold">{u.fullName} <span className="text-xs font-normal text-slate-500">({u.role}{u.role === "COLLECTOR" ? ` â€¢ ${u.activeJobs} active` : ""})</span></p><p className="text-xs text-slate-500">{u.email} â€¢ {u.points} pts â€¢ {u.isActive ? "active" : "disabled"}</p></div>
