...
export default function AuditPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-6">
      ...
      <div className="mt-4 grid gap-2">{logs.map((l) => <Card key={l.id}><p className="text-sm font-bold">{l.action} — {l.entity} {l.entityId?.slice(0, 8)}</p><p className="text-xs text-slate-500">{l.actor?.fullName ?? "system"} • {new Date(l.createdAt).toLocaleString()}</p></Card>)}</div>
    </main>
  );
}
